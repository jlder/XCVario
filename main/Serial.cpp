...
void Serial::taskStart(){
	...
	if( serial1 ){
		xTaskCreatePinnedToCore(&serialHandler, "serialHandler1", 6144, &S1, 13, &S1.pid, 0);  // stay below canbus
	}
	if( serial2 ){
		xTaskCreatePinnedToCore(&serialHandler, "serialHandler2", 6144, &S2, 13, &S2.pid, 0);  // stay below canbus
	}
}

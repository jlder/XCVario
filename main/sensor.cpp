...
void readSensors(void *pvParameters){
	...
	// initialize Altitude alpha/beta filter
	ALT = (1.0 - pow( (statP-(QNH.get()-1013.25)) * 0.000986923 , 0.1902891634 ) ) * (273.15 + OATemp) * 153.846153846;
	...
	while (1)
	{
		...
		/* Sensor data
			...
			Roll in milli rad,
			Yaw in milli rad,
			Free Pitch in milli rad,
			Free Roll in milli rad,
			Free Yaw in milli rad,
...
void system_startup(void *args){
	...
	if( SetupCommon::isClient() ){
		...
		xTaskCreatePinnedToCore(&readSensors, "readSensors", 5120, NULL, 15, &bpid, 0);
